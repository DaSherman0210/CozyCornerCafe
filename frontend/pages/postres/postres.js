...
async function getCards () {
  ...
  postres.forEach((postre)=>{
    const {nombre,precio,descripcion,imagen} = postre;
    cardsContainer.innerHTML += `
    <div class="swiper-slide" style="background-image: url(../../assets/imgs/postres/${imagen}); background-repeat: no-repeat; background-size: cover; background-position:center;">
        <span>${nombre}</span>
        <div>
          ...
          <a class="postresButton btn btn-danger" precio="${precio}" nombre="${nombre}" href="../carrito/carrito.html">Comprar</a>
        </div>
      </div>
  `
  })
...
cardsContainer.addEventListener('click', guardarPostres);
let arrayCarro = [];

function guardarPostres(e) {
  e.preventDefault();
  if (e.target.classList.contains('postresButton')) {
    ...
    localStorage.setItem("postres", JSON.stringify(arrayCarro));
